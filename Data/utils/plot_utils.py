...
def plot_tsp(p, x_coord, W, W_val, W_target,height ,title="default"):
    """
    ...
    labels = dict(zip(range(len(x_coord)), height  ))
    
    nx.draw_networkx_nodes(G, pos, node_color=colors, node_size=40,ax=p)
    nx.draw_networkx_edges(G, pos, edgelist=adj_pairs, alpha=0, width=1,ax=p)
    nx.draw_networkx_edges(G, pos, edgelist=target_pairs, alpha=1, width=2, edge_color='r',ax=p)
    pos_higher = {}
    for k, v in pos.items():
        if(v[1]>0):
            pos_higher[k] = (v[0]-2, v[1]+6)
            #pos_higher[k] = (v[0]-10, v[1]+50)#pos_higher[k] = (v[0]-2, v[1]+6)
        else:
            pos_higher[k] = (v[0]-0.1, v[1]-0.5) #pos_higher[k] = (v[0]-10, v[1]-40)#pos_higher[k] = (v[0]-0.1, v[1]-0.5)
   ...
   # nx.relabel_nodes(G, labels)
    p.set_title(title)
    p.tick_params(left=True, bottom=True,labelleft=True,labelbottom=True,labelsize=14,length=6, width=6)
    #p.set_xticks([0,0.1,0.2])
    #p.set_xlim([0, 1])
